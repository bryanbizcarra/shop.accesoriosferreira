...
class CartPriceManager {
	...
	public function calculateTotals( $cart ) {
		...
		if ( ! empty( $cart->cart_contents ) ) {
			foreach ( $cart->cart_contents as $key => $cart_item ) {
				...
				$needPriceRecalculation = apply_filters( 'tier_pricing_table/cart/need_price_recalculation', true,
					$cart_item );

				if ( $cart_item['data'] instanceof WC_Product && $needPriceRecalculation ) {

					$product_id = ! empty( $cart_item['variation_id'] ) ? $cart_item['variation_id'] : $cart_item['product_id'];
					$new_price  = PriceManager::getPriceByRules( $this->getTotalProductCount( $cart_item ), $product_id, 'calculation', 'cart' );
					$new_price  = apply_filters( 'tier_pricing_table/cart/product_cart_price', $new_price, $cart_item );
	...
	public function calculateItemPrice( $price, $cart_item ) {
		...
		if ( $cart_item['data'] instanceof WC_Product && $needPriceRecalculation ) {
			...
			$new_price = PriceManager::getPriceByRules( $this->getTotalProductCount( $cart_item ), $cart_item['data']->get_id(), 'view', 'cart' );
			...
			$regular_price = apply_filters( 'tier_pricing_table/cart/product_cart_regular_price/item', wc_get_price_to_display( $product ), $cart_item );

			if ( $new_price !== false ) {

				if ( $this->settings->getAll()['show_discount_in_cart'] === 'yes' && tpt_fs()->is_premium() ) {
